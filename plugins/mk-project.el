...
(defun mk-proj-load-vars (proj-name proj-alist)
  ...
  (labels ((config-val (key)
    ...
    ;; optional vars
    (dolist (v '(src-patterns ack-args vcs subdir
                 compile-cmd src-find-cmd grep-find-cmd
...
(defun project-find-tag (&optional phrase)
  (interactive)
  (let* (
         (count 0)
         (wap (word-at-point))
         ;(regexp (or phrase (if wap (read-string (concat "Find tag for: (default \"" wap "\"): ") nil nil wap)
          ;                    (read-string "Find tag for: "))))
         (regexp (if phrase (read-string "Find tag for: ") 
                (if wap wap (read-string (concat "Find tag for: (default \"" wap "\"): ") nil nil wap))
